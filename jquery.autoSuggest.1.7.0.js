...
/**
 * @modifer: nghia.luong@sonicviet.com
 * @version: 1.7.0
 * ======================================================================
 *
 * mở rộng function autoSuggest cho phép tìm kiếm gần đúng theo tiếng việt
 * options: matchUTF
 * + nếu matchWeak là true cho phép tìm kiếm gần đúng theo tiếng việt
 * + mặc định matchWeak là false sẽ thực hiện tìm kiếm chính xác từ cần tìm
 * ví dụ:
 *@ các từ có dấu sẽ tương đương với vơi từ gốc
 * a á à ạ ả ã 	~ a
 * e é è ẹ ẻ ẽ 	~ e
 * i í ì ị ỉ ĩ 	~ i
 * u ú ù ụ ủ ũ 	~ u
 ...
 * y ý ỳ ỵ ỷ ỹ 	~ y
 *
 * @ Các nguyên âm mở rộng trong tiếng việt sẽ được sử dụng như từ gốc
 * a ă â 	~ a
 * o ơ ô 	~ o
 * u ư		~ u
 *
	...
	$.fn.autoSuggest = function(data, options) {
		...
		if (d_fetcher) {
			return this.each(function(x) {
				...
				var num_count = 0;
				function processWeakQuery(query) {
					//init array utf-8 vietnamese
					var patterns = [//
					"(o|ọ|ó|ò|õ|ỏ|ô|ộ|ố|ồ|ỗ|ổ|ơ|ợ|ớ|ờ|ỡ|ở)", // ~ o
					"(a|ạ|á|à|ã|ả|ă|ặ|ắ|ằ|ẵ|ẳ|â|ậ|ấ|ầ|ẫ|ẩ)", // ~ a
					"(e|ẹ|é|è|ẽ|ẻ|ê|ệ|ế|ề|ễ|ể)", // ~ e
					"(i|ị|í|ì|ĩ|ỉ)", // ~ i
					"(u|ụ|ú|ù|ũ|ủ|ư|ự|ứ|ừ|ữ|ử)", //~ u
					"(y|ỵ|ý|ỳ|ỹ|ỷ)"// ~ y
					];
					$.each(patterns, function(i, pt) {
						var patt = new RegExp(pt, "gi");
						query = query.replace(patt, pt);
					})
					return query;
				}
				...
				function processData(data, query) {
					...
					for (var i = 0; i < d_count; i++) {
						...
						if (str) {
							...
							//sử dụng regExp để tìm kiếm string match thay vì sử dụng search
							if (regx.test(str) && values_input.val().search("," + data[num][opts.selectedValuesProp] + ",") == -1) {
								forward = true;
							}
						}
						if (forward) {
